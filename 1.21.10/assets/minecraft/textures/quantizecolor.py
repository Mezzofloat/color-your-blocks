...
def meanShift(image_path):
    print(image_path)
    test = cv2.cvtColor(cv2.imread(image_path), cv2.COLOR_BGR2RGBA)
    ...
    cv2.imwrite(image_path, colored_segmented_image)

for file in Path('../textures').rglob("*.png"):
    meanShift(file)